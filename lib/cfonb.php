...
//$rib_titulaire = str_replace(",", "", $rib['titulaire']);
//$rib_banque = str_replace(",", "", $rib['banque']);
...
$ligne .= FormatBancaire(stripAccents(' '), 20); // Domicialiation : facultatif (24 caractères)
$ligne .= FormatBancaire("", 12); // Déclaration à la balance des paiements : ??????? (8 caractères)
$ligne .= FormatBancaire($rib['guichet'], 5); // Code guichet bénéficiaire (5 caractères)
$ligne .= FormatBancaire($rib['compte'], 11); // Numéro de compte bénéficiaire (11 caractères)
...
$ligne .= FormatBancaire($rib['banque'], 5); // Code établissement bénéficiaire (5 caractères)
$ligne .= FormatBancaire("", 6); // Zone réservée (6 caractères)
...
function GetRibFromIban($iban) {
	$chars = substr($iban, 4, 24);
	
	$val['banque']	= substr($chars, 0, 5);
	$val['guichet']	= substr($chars, 5, 5);
	$val['compte']	= substr($chars, 10, 11);
	$val['cle']		= substr($chars, 21, 2);
