...
function update_ca() {
  ...
  mysql_query("UPDATE webcash_clients SET ca_total_ht=0 where ca_total_ht!=0");
  $result = mysql_query("SELECT f.id_client as id_client,round(sum(fl.qtt*fl.prix_ht),0) as ca_total_ht
                          FROM webcash_invoice_rows as fl, webcash_invoices as f
  ...
  while ($ca = mysql_fetch_object($result)) {
    $q = sprintf("UPDATE webcash_clients SET ca_total_ht='%.2f' WHERE id_client=%d",
  ...
  mysql_query("UPDATE webcash_clients SET ca_total_ht_year=0 where ca_total_ht_year!=0");
  $result = mysql_query("SELECT f.id_client as id_client,round(sum(fl.qtt*fl.prix_ht),0) as ca_total_ht_year
                          ...
                          GROUP BY f.id_client") or die(mysql_error());
  while ($ca = mysql_fetch_object($result)) {
    $q = sprintf("UPDATE webcash_clients SET ca_total_ht_year='%.2f' WHERE id_client=%d",
  ...
  // TOTAL DU HT
  mysql_query("UPDATE webcash_clients SET total_du_ht=0");
  ...
  while ($du = mysql_fetch_object($result)) {
    $q = sprintf("UPDATE webcash_clients SET total_du_ht='%.2f' WHERE id_client=%d", $du->total_du_ht, $du->id_client );
    mysql_query($q) or die(mysql_error());
  }
}

function regenerate($id) {
  mysql_query("UPDATE webcash_invoices SET date_generated=NULL,facture_file=NULL where id_facture=$id");
}

function renum() {
  $result = mysql_query("SELECT id_facture FROM webcash_invoice_rows");
  while (list($id_facture) = mysql_fetch_array($result)) {
    $count = 1;
    $result2 = mysql_query("SELECT id_facture_ligne FROM webcash_invoice_rows WHERE id_facture=$id_facture ORDER BY ordre");
    while (list($id_facture_ligne) = mysql_fetch_array($result2)) {
      mysql_query("UPDATE webcash_invoice_rows SET ordre=$count WHERE id_facture_ligne=$id_facture_ligne");
      $count += 2;
    }
...
#  print_r($_POST);
  ...
  if (($facture->is_envoye == 0) && ($is_envoye == "on")) {
    $result = mysql_query("SELECT count(*) FROM webcash_invoices WHERE num_facture!='' AND id_facture!=".$facture->id_facture ." AND year(date_facture)=year('".$facture->date_facture."')") or die(mysql_error());
  ...
  $q = sprintf("UPDATE webcash_invoices SET type_paiement='%s',is_paye=%d,%s ref_contrat='%s', extra_top='%s', extra_bottom='%s', accompte='%s', date_facture='%s', type_doc='%s', commentaire='%s', id_type_presta=%d, id_compte=%d, is_envoye=%d, num_facture='%s'
                WHERE id_facture='%d'",
  ...
  if ((is_numeric($_POST['prix_ht_new'])) && (is_numeric($_POST['qtt_new'])) && ($_POST['prix_ht_new'] > 0) && !empty($_POST['line_new'])) {
    ...
    $q = sprintf("INSERT INTO webcash_invoice_rows (id_facture,description,prix_ht,qtt) VALUES(%d, '%s', '%s', '%s')",
                 $_POST['id_facture'], $_POST['line_new'], $_POST['prix_ht_new'], $_POST['qtt_new'] );
    $result = mysql_query($q) or die(mysql_error());
    mysql_query("UPDATE webcash_invoices SET date_generated=NULL WHERE id_facture=".$_POST['id_facture']);
  }

  // Enregistrement des lignes existantes
  foreach ($_POST as $k=>$v) {
    if (preg_match("/^line_([0-9]+)$/", $k, $matches)) {
      $q = sprintf("UPDATE webcash_invoice_rows SET description='%s', prix_ht='%s', qtt='%s' WHERE id_facture_ligne=%d",
  ...
  if (preg_match("/^raise:([0-9]+)$/", $_POST['raise_lower'], $matches)) {
    mysql_query("UPDATE webcash_invoice_rows SET ordre=ordre-3 WHERE id_facture_ligne=".$matches[1]) or die(mysql_error());
    renum();
  }
  ...
  if (preg_match("/^delete:([0-9]+)$/", $_POST['raise_lower'], $matches)) {
    mysql_query("DELETE FROM webcash_invoice_rows WHERE id_facture_ligne=".$matches[1]) or die(mysql_error());
    renum();
  }

  $q = sprintf("UPDATE webcash_clients SET vat_number='%s' WHERE id_client=%d", $vat_number, $facture->id_client);
  mysql_query($q);
  ...
  $result = mysql_query("SELECT id_client FROM webcash_invoices WHERE id_facture=".$_GET['id_facture']);
  list($id_client) = mysql_fetch_array($result);
  mysql_free_result($result);

  logmessage("Suppression d'une facture pour client:$id_client");

  mysql_query("DELETE FROM webcash_invoices WHERE id_facture=".$_GET['id_facture']);
  mysql_query("DELETE FROM webcash_invoice_rows WHERE id_facture=".$_GET['id_facture']);
  ...
  $result = mysql_query("SELECT id_client FROM webcash_invoices WHERE id_facture=$id");
  list($id_client) = mysql_fetch_array($result);
  mysql_free_result($result);

  mysql_query("INSERT INTO webcash_invoices (id_client,date_created,date_facture) VALUES($id_client, now(), now())") or die(mysql_error());
  $result = mysql_query("SELECT id_facture FROM webcash_invoices WHERE id_client=$id_client AND date_sub(now(), INTERVAL 2 SECOND)<date_created");
  list($id_new_facture) = mysql_fetch_array($result);
  mysql_free_result($result);

  // On recopie les données de la facture
  mysql_query("UPDATE webcash_invoices as f1, webcash_invoices as f2
               SET
  ...
  mysql_query("INSERT INTO webcash_invoice_rows (id_facture,description,qtt,ordre,prix_ht) SELECT $id_new_facture,description,qtt,ordre,prix_ht FROM webcash_invoice_rows WHERE id_facture=$id") or die(mysql_error());
