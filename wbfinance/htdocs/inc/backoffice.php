...
function parselogline($str) {
  if (preg_match("/(user|fa|client):([0-9]+)/", $str)) {
    while (preg_match("/(user|fa|client):([0-9]+)/", $str, $matches)) {
      switch ($matches[1]) {
        case "fa":
          $result = mysql_query("SELECT num_facture FROM facture WHERE id_facture=".$matches[2]);
          ...
          break;
        case "user":
          $result = mysql_query("SELECT login FROM user  WHERE id_user=".$matches[2]);
          list($login) = mysql_fetch_array($result);
          mysql_free_result($result);
          $str = preg_replace("/".$matches[0]."/", '<a href="/admin/fiche_user.php?id='.$matches[2].'">'.$login.'</a>', $str);
          break;
        case "client":
          $result = mysql_query("SELECT nom FROM client WHERE id_client=".$matches[2]);
...
function logmessage($msg) {
  ...
  $msg = preg_replace('/"/', "\\'", $msg );
  mysql_query("INSERT INTO userlog (log,date,id_user) VALUES('$msg', now(), $id)") or die(mysql_error());
}
